                ...
                >
                    ...
                    {proyectosFiltrados.length > 0 && (
                        <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                            {proyectosFiltrados.map(proyectosFiltrado => (
                                <Combobox.Option key={proyectosFiltrado._id} value={proyectosFiltrado} className={({active}) => classNames("cursor-default select-none px-4 py-2", active && "bg-sky-600 text-white")}>{proyectosFiltrado.nombre}</Combobox.Option>
                            ))}
                        </Combobox.Options>
                    )}
                    </Combobox>
